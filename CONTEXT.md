...
| `02_validate/` | a tickets file and the snapshot it names | nothing; an exit code and coded failure lines. `validate.py` is a frame with the reading stage, the pairing phase, canonical form and grammar, and the row states written, and every other check registered and empty | that the exit code is 0 before the tickets are used |
