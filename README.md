...
| `examples.md` | three worked pairs — an example snapshot and the tickets file written for it, input then output, each a byte copy in a fenced block; generated by the script in `03_examples/` from its manifest, committed, and never edited by hand |
...
| `00_fetch/` | step 00: `fetch.py`, one URL or a file of them to numbered, hashed snapshots; ten snapshots in `00_snapshots/`, three of them the worked examples |
| `01_translate/` | step 01: the translation, done by Claude; its answers go in `00_tickets/`, which holds a tickets file for each snapshot, ten in all |
...
next five follow one step or tool each, every decision it makes and every way it ends; the last
shows which code reads which contract table. `stem` is a snapshot's file name without `.txt`.
Scripts are code and call no model; the translation is Claude's.
...
flowchart TB
    ...
    subgraph S3["03_examples/ · code"]
        EXM[("examples-manifest.md<br/>3 example pairs")]
...
flowchart TB
    subgraph SUITE["python3 02_validate/run_fixtures.py"]
        ...
        EXM[("03_examples/examples-manifest.md")] --> PAIR["each example pair: validate.py must exit 0,<br/>print nothing, and the header name the row's snapshot"]
...
flowchart LR
    ...
    CON -->|"header-items · fields ·<br/>schema-constants"| CR
    CON -->|"checks"| RF
...
the exit is 0. For example, the PagerDuty example snapshot shipped here was fetched from:
...
Run as written, this adds another PagerDuty file beside the PagerDuty snapshots already there, named
with the time of your fetch: that file is the snapshot to translate in step 2. To try fetch without
...
plain-text file, as the three example snapshots are:
...
each was fetched from is the first line of its header. Seven more snapshots stand beside them,
fetched on 2026-09-25 from branch URLs, so the pages behind them may have changed since; one of
them, Slack's, is an HTML page reduced to text. In Twilio's, every one of the 88 `entry_date` rows
reads `not in source`: its dates stand on setext headings, the limit **Limits** names below.
`00_fetch/00_snapshots/CONTEXT.md` lists them.
...
snapshot's file name without `.txt`. For the PagerDuty example snapshot,
...
The three example snapshots were translated this way, each in a headless session (`claude -p`)
...
`01_translate/00_tickets/`, never edited after the model wrote them. The tickets files of the seven
other snapshots stand beside them: on 2026-09-25 a Claude Code session was given each URL and asked
to fetch it and write its tickets, did both, and no file was edited by hand after. The body ranges
of the two long inputs it translated, Twilio 1-250 and Zulip 21-129, were chosen in that session,
and the other four long inputs were left as refusals. Which model it ran on, and whether the hooks
were live in it, is not recorded. `rules.md` says what it has not settled.

A Claude Code session that is already running somewhere else — the one that cloned this repository,
say — translates the same way, without a new session: it reads `CONTEXT.md`, section **Translating
...
`03_examples/examples-manifest.md` — the three example pairs — and one for `examples.md`, all
`pass`: each pair's tickets file is run through the validator with the snapshot folder of the
repository and must exit 0 and print nothing, a warning included; its header must name the row's
snapshot; and `examples.md` is regenerated into a temporary directory, which is deleted afterwards,
and must equal the committed file byte for byte. A `fail` on any of the four lines fails the suite
like a fixture's; the `examples.md` line then names the first differing byte and its line. Nothing
is written into the repository. A tickets file in `01_translate/00_tickets/` that no manifest row
...
script, the manifest or an example pair means running it again and committing what it writes, and
...
or later and are skipped below it. Nobody has run 3.10 to 3.13. The three example snapshots are
found through `03_examples/examples-manifest.md`, so a snapshot you fetched beside them in step 1
changes no result. The third needs no network: it runs `fetch.py` against a stub server on
127.0.0.1. The fourth is the negative test of the hook wrapper: it feeds the wrapper hook input in a
temporary folder, under `/bin/sh` and under `dash` when it is on PATH. The fifth holds the examples
script, every output in a temporary directory. `python3 lib/idemlib/contract.py` loads the contract
on its own and prints `16 tables, 221 rows, named by reference/00_catalogue.md` last, with exit 0.
...
1. **Upload these eight files**, flat, as they are named; they cite one another by bare name:
   ...
   `reference/`. Together they are 235,891 bytes, measured on 2026-09-25; it changes when those
...
a recommendation. Each of the three example snapshots was pasted as steps 4 and 5 say, in three
chats of its own. Every answer was complete and passed the validator once what the Copy button
brings was removed, and for PagerDuty (64 tickets) and Plaid (102) the three answers gave one shape
by `02_validate/compare_runs.py`. Docker's first three did not: one answer read two wrapped items
and one version-bound sentence differently from the other two, the contract gained the two sentences
...
## What is not built or not yet observed
...
- **Nothing forbids writing a tickets file with a script.** The translation is Claude's, not a
  ...
  Claude Code session that wrote the three example tickets files first tried to generate its file
  with a script of its own, and what stopped it was the headless session's permissions and the
...
deny hook of `.claude/` denies the file tools any write to that folder.
