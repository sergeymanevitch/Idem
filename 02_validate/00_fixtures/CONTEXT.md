...
**Every row of `manifest.md` is written; thirty-five files exist — thirty tickets files and five
...
about it; `refusal_reason-01`, which is not a mutation of it; and twenty-eight mutations of it that
the reading stage, the pairing phase, canonical form and grammar, and the row states catch. The
remaining thirty-four tickets files come with the checks that read them, and `../run_fixtures.py`
...
**One snapshot was fetched; the other four were built by hand from it.**
`changelog-01.txt` is evidence: the Plaid snapshot of `../../00_fetch/00_snapshots/`, byte for
byte. The four beside it were never fetched from anything and no server ever served them — they
...
citing it can be over the size limit while its pairing still passes. Nothing in this folder is a
record of a page as it was served, and nothing here should ever be read as one.
...
| `01_tickets/*-<nn>.tickets.md` | one mutation of `clean-01` each, named for the check it must raise |
