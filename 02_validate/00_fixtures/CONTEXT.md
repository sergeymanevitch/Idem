...
**Every row of `manifest.md` is written; sixty-nine files exist — sixty-one tickets files and eight
...
about it; `clean-03`, the zero-ticket shape over the same snapshot — its header, no ticket, and
every non-blank body line listed with its text — that must be accepted the same way; `refusal_reason-01`,
which is not a mutation of either; and fifty-eight mutations that the reading stage, the pairing
phase, canonical form and grammar, the row states, quotes and values, ranges and ancestors and
coverage catch, or that one of the two warnings points at. Forty-nine of those mutate `clean-01`
and one mutates `clean-03`; the three built over `terms-01.txt` mutate `clean-01` moved onto that
snapshot, the three built over `items-01.txt` mutate `clean-01` moved onto that one, and the two
built over `warns-01.txt` mutate `clean-01` moved onto that one — three bases the corpus does not
carry, because each would be a second clean file nothing names, and each rebuilt and run by a test
instead. The remaining three tickets files come with the checks that read them, and
`../run_fixtures.py` prints how many are still waiting.
...
**One snapshot was fetched; the other seven were built by hand from it.**
...
| `01_tickets/*-<nn>.tickets.md` | one mutation each of a clean file — `clean-01`, or `clean-03` for the zero-ticket mutation, or `clean-01` moved onto `terms-01.txt` for the three that need a listed phrase, onto `items-01.txt` for the three that need a continuation line, or onto `warns-01.txt` for the two warnings — named for the check it must raise |
