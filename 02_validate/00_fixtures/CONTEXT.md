...
**Every row of `manifest.md` is written; fifty-seven files exist — fifty tickets files and seven
...
about it; `refusal_reason-01`, which is not a mutation of it; and forty-eight mutations that the
reading stage, the pairing phase, canonical form and grammar, the row states, quotes and values,
and ranges and ancestors catch. Forty-two of those mutate `clean-01`; the three built over
`terms-01.txt` mutate `clean-01` moved onto that snapshot, and the three built over `items-01.txt`
mutate `clean-01` moved onto that one — two bases the corpus does not carry, because each would be
a second clean file nothing names, and each rebuilt and run by a test instead. The remaining
fourteen tickets files come with the checks that read them, and `../run_fixtures.py` prints how many
are still waiting.
...
**One snapshot was fetched; the other six were built by hand from it.**
`changelog-01.txt` is evidence: the Plaid snapshot of `../../00_fetch/00_snapshots/`, byte for
byte. The six beside it were never fetched from anything and no server ever served them — they
...
ranges-and-ancestors phase need both. Nothing in this folder is a record of a page as it was
served, and nothing here should ever be read as one.
...
| `01_tickets/*-<nn>.tickets.md` | one mutation each of a clean file — `clean-01`, or `clean-01` moved onto `terms-01.txt` for the three that need a listed phrase, or onto `items-01.txt` for the three that need a continuation line — named for the check it must raise |
