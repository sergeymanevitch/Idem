...
states and quotes and values are written — all of that last phase but the row that searches a quote
in a supplied input text — and every other row is registered with nothing behind it.
`02_validate/run_fixtures.py` runs the fixture corpus, of which forty-eight files exist — forty-two
tickets files and six snapshots.**
...
canonical form and grammar, the row states, five of the six rows of quotes and values, and the
warning about the unnumbered mode; the two phases below them are registered and empty, and the
story that fills each one writes its checks into that file and nowhere else. The routine of
...
against `05_checks.md` by `02_validate/test_manifest.py`; forty-two of the tickets files it names
exist — one clean file over the Plaid snapshot, one refusal built from its header, and one mutation
per row of the five written phases — beside six snapshots, of which one is the fetched Plaid file
and five were built by hand from it.
