...
Nothing here is a check. `validate.py` is not written, and the file itself says that a row of
`checks` states what will be checked and under what code, never that anything checks it. What can
be proved today is that the table is well formed, that it holds the rows the story requires, that
